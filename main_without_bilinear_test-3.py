...
file_name = EV_data_file + 'my_list_8_test_save.pkl'
# file_name = EV_data_file + 'my_list_93_save.pkl'
with open(file_name, 'rb') as fo:
    my_list_last = pickle.load(fo)

file_name = EV_data_file + 'n_car_number_2evcs_test_save.pkl'
...
assert len(my_list_last) <= total_time
assert len(car_already_in) == total_time
...
if solve:
    ...
    evcssp_model.update()
    evcssp_model.read("evcssp_out.sol")
