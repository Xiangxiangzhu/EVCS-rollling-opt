...
for t in time:
    if t >= 1:
        ...
        energy_consumption[t] = evcssp_model.addConstr(
            energy[t] == gp.quicksum(soc[t, m] - soc[(t - 1), m] for tt, m in my_car_index if (tt == t)),
...
for t, m in my_car_index:
    transformer_limit_2[t, m] = evcssp_model.addGenConstrPWL(limit_temp_star[t, m], limit_temp_square[t, m],
                                                             [-0.9, 0, 0, 5, 9.32, 10.66, 12, 13.34, 14.68, 14.68, 15],
                                                             [0, 0, 6, 6.105, 6.333, 4.739, 3.596, 2.675, 1.968, 0, 0],
